...
export class UserMapper implements IDbMapper<User, PrismaUserWithRoles> {
    toDomain(dbUser: PrismaUserWithRoles): Result<User> {
        ...
        if (dbUser.authProvider === 'LOCAL') {
            authProviderVo = AuthProvider.createLocal(
                dbUser.password!,
            ).getValue();
        } else {
    ...
    toPersistence(user: User): unknown {
