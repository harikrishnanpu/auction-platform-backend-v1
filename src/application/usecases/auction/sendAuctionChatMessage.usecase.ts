...
export class SendAuctionChatMessageUsecase implements ISendAuctionChatMessageUsecase {
    ...
    async execute(
        ...
        const messageEntityResult = AuctionChatMessage.create({
            id: messageId,
        ...
        if (messageEntityResult.isFailure) {
            return Result.fail(messageEntityResult.getError());
        }

        const msg = messageEntityResult.getValue();

        const saveResult = await this._chatRepository.create(msg);
